...
class GEXCache:
    ...
    def invalidate_on_spot_move(
        self, old_spot: float, new_spot: float, threshold_pct: float = 0.01
    ) -> bool:
        ...
        if move_pct > threshold_pct:
            # Invalidate all GEX-related caches
            count = self.invalidate_prefix("gex:")
            count += self.invalidate_prefix("options:")
            logger.info(
                f"Spot moved {move_pct:.2%} ({old_spot:.2f} → {new_spot:.2f}), "
    ...
    def update_spot_price(self, spot_price: float) -> bool:
